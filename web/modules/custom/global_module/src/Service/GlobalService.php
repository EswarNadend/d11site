...
/**
 * Provides global configuration variables from Vault.
 */
class GlobalService
{
    ...
    /**
     * Constructs a GlobalService object.
     *
    ...
    /**
     * Fetch and return global variables from Vault.
     *
     * @return array|null
     *   An array of global config data, or NULL on failure.
     */
    public function getGlobalVariables(): ?array
    {
        try {
            ...
            $response = $this->httpClient->get($vaultAPI, [
                'headers' => [
            ...
            if ($data) {
                ...
                if (isset($decoded['data'])) {
                    $vaultData = (array) $decoded['data'];

                    // Add helpful shortcuts to top level of array.
            ...
            $this->logger->error('Vault fetch failed: @msg', ['@msg' => $e->getMessage()]);
        }

        return NULL;
    }

    public function getApimanAccessToken()
    {

        $globalVariables = $this->getGlobalVariables();

        $client = \Drupal::httpClient();
        $tokenUrl = $globalVariables['apiManConfig']['config']['apiUrl']
            . 'tiotAPIESBSubSystem'
        ...
        try {
            $response = $client->post($tokenUrl, [
                'json' => $globalVariables['apiManConfig']['config'], // Send JSON properly
            ]);
            $data = json_decode($response->getBody(), true);
            return $data['access_token'] ?? NULL;
        } catch (\Exception $e) {
            \Drupal::logger('global_module')->error('Access token fetch failed: @message', ['@message' => $e->getMessage()]);
            return NULL;
        }
    }
